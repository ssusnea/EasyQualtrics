...
detect_likert <- function(col) { # looks at a single chr vector
  ...
  if (TRUE %in% all(grepl("agree", levels(col)))) { # looks for key string (ex "agree")
  ...
  if (TRUE %in% all(grepl("satisfied", levels(col)))) { # new key word: "satisfied"
  ...
  if (TRUE %in% all(grepl("appropriate", levels(col)))) { # new key word: "appropriate"
  ...
  if (TRUE %in% (grepl("true", levels(col)))) { # new key word: "true"
